...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\dbrau\\Dropbox\\post_doc\\professional\\projects\\independent\\gradcptpy\\gradcptpy_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    prepare_block_text = visual.TextStim(win=win, name='prepare_block_text',
        text='',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
    ...
    for thisBlock in blocks:
        ...
        prepare_block = data.Routine(
            name='prepare_block',
            components=[prepare_block_text],
        )
        prepare_block.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from prepare_block_code
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            # Run 'Each Frame' code from prepare_block_code
            timer = round(5 - (datetime.now() - prepare_start).total_seconds())
            
            # *prepare_block_text* updates
            
            # if prepare_block_text is starting this frame...
            if prepare_block_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                prepare_block_text.frameNStart = frameN  # exact frame index
                prepare_block_text.tStart = t  # local t and not account for scr refresh
                prepare_block_text.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(prepare_block_text, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'prepare_block_text.started')
                # update status
                prepare_block_text.status = STARTED
                prepare_block_text.setAutoDraw(True)
            
            # if prepare_block_text is active this frame...
            if prepare_block_text.status == STARTED:
                # update params
                prepare_block_text.setText('The next images will appear in {} seconds.'.format(timer), log=False)
            
            # if prepare_block_text is stopping this frame...
            if prepare_block_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > prepare_block_text.tStartRefresh + 5-frameTolerance:
                    # keep track of stop time/frame for later
                    prepare_block_text.tStop = t  # not accounting for scr refresh
                    prepare_block_text.tStopRefresh = tThisFlipGlobal  # on global time
                    prepare_block_text.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'prepare_block_text.stopped')
                    # update status
                    prepare_block_text.status = FINISHED
                    prepare_block_text.setAutoDraw(False)
        ...
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if prepare_block.maxDurationReached:
            routineTimer.addTime(-prepare_block.maxDuration)
        elif prepare_block.forceEnded:
            routineTimer.reset()
        else:
            routineTimer.addTime(-5.000000)
        ...
        for thisTrial in trials:
            ...
            # create an object to store info about Routine prepare_trial
            prepare_trial = data.Routine(
                name='prepare_trial',
                components=[prepare_text],
            )
            prepare_trial.status = NOT_STARTED
            continueRoutine = True
            # update component parameters for each repeat
            ...
            timer = 5
            # store start times for prepare_trial
            prepare_trial.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
            prepare_trial.tStart = globalClock.getTime(format='float')
            prepare_trial.status = STARTED
            thisExp.addData('prepare_trial.started', prepare_trial.tStart)
            prepare_trial.maxDuration = None
            # keep track of which components have finished
            prepare_trialComponents = prepare_trial.components
            for thisComponent in prepare_trial.components:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            # --- Run Routine "prepare_trial" ---
            # if trial has changed, end Routine now
            if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
                continueRoutine = False
            prepare_trial.forceEnded = routineForceEnded = not continueRoutine
            while continueRoutine and routineTimer.getTime() < 5.0:
                # get current time
                t = routineTimer.getTime()
                tThisFlip = win.getFutureFlipTime(clock=routineTimer)
                ...
                # check if all components have finished
                if not continueRoutine:  # a component has requested a forced-end of Routine
                    prepare_trial.forceEnded = routineForceEnded = True
                    break
                continueRoutine = False  # will revert to True if at least one component still running
                for thisComponent in prepare_trial.components:
                    if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
            ...
            for thisComponent in prepare_trial.components:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            # store stop times for prepare_trial
            prepare_trial.tStop = globalClock.getTime(format='float')
            prepare_trial.tStopRefresh = tThisFlipGlobal
            thisExp.addData('prepare_trial.stopped', prepare_trial.tStop)
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
            if prepare_trial.maxDurationReached:
                routineTimer.addTime(-prepare_trial.maxDuration)
            elif prepare_trial.forceEnded:
                routineTimer.reset()
            else:
                routineTimer.addTime(-5.000000)
            thisExp.nextEntry()
        ...
        write_data(es_gradcpt_data, str(int(blocks.thisN)).zfill(2))
        write_data(gradcpt_data, str(int(blocks.thisN)).zfill(2))
